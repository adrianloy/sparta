...
class Controller():
    ...
    def initScreenshooter(self):
        self.screenshooter = Screenshooter(
            self.settings.general_screenshooter_timeout)  # screenshot taker object (different thread)
    ...
    def initTimers(
            self):  # these timers are used to prevent from updating the UI several times within a short time period - which freezes the UI
    ...
    def loadSettings(self):
        ...
        self.originalSettings = Settings(
            self.settingsFile)  # save the original state so that we can know if something has changed when we exit SPARTA
    ...
    def applySettings(self,
                      newSettings):  # call this function when clicking 'apply' in the settings menu (after validation)
        print '[+] Applying settings!'
        self.settings = newSettings

    def cancelSettings(self):  # called when the user presses cancel in the Settings dialog
        self.view.settingsWidget.setSettings(
            self.settings)  # resets the dialog's settings to the current application settings to forget any changes made by the user
    ...
    def addHosts(self, iprange, runHostDiscovery, runStagedNmap):
        ...
        if runStagedNmap:
            ...
            self.runCommand('nmap', 'nmap (discovery)', iprange, '', '', command, getTimestamp(True), outputfile,
                            self.view.createNewTabForHost(str(iprange), 'nmap (discovery)', True))

        else:
            outputfile = self.logic.runningfolder + "/nmap/" + getTimestamp() + '-nmap-list'
            command = "nmap -n -sL " + iprange + " -oA " + outputfile
            self.runCommand('nmap', 'nmap (list)', iprange, '', '', command, getTimestamp(True), outputfile,
                            self.view.createNewTabForHost(str(iprange), 'nmap (list)', True))

    #################### CONTEXT MENUS ####################

    def getContextMenuForHost(self, isChecked,
                              showAll=True):  # showAll exists because in some cases we only want to show host tools excluding portscans and 'mark as checked'
    ...
    def handleHostAction(self, ip, hostid, actions, action):
        ...
        if action.text() == 'Run nmap (staged)':
            print '[+] Purging previous portscan data for ' + str(
                ip)  # if we are running nmap we need to purge previous portscan results
        ...
        for i in range(0, len(actions)):
            if action == actions[i]:
                ...
                outputfile = self.logic.runningfolder + "/" + re.sub("[^0-9a-zA-Z]", "",
                                                                     str(name)) + "/" + getTimestamp() + "-" + re.sub(
                    "[^0-9a-zA-Z]", "", str(self.settings.hostActions[i][1])) + "-" + ip
                ...
                if 'nmap' in command:
                    ...
                    if self.logic.getPortsForHostFromDB(ip,
                                                        proto):  # if we are running nmap we need to purge previous portscan results (of the same protocol)
    ...
    def getContextMenuForServiceName(self, serviceName='*', menu=None):
        ...
        for a in self.settings.portActions:
            if serviceName is None or serviceName == '*' or serviceName in a[3].split(",") or a[
                3] == '':  # if the service name exists in the portActions list show the command in the context menu
                actions.append([self.settings.portActions.index(a), menu.addAction(
                    a[0])])  # in actions list write the service and line number that corresponds to it in portActions
        ...
        for i in range(0, len(actions)):
            if action == actions[i][1]:
                ...
                for ip in targets:
                    ...
                    outputfile = self.logic.runningfolder + "/" + re.sub("[^0-9a-zA-Z]", "", str(
                        tool)) + "/" + getTimestamp() + '-' + tool + "-" + ip[0] + "-" + ip[1]
    ...
    def handlePortAction(self, targets, actions, terminalActions, action, restoring):
        ...
        if action.text() == 'Send to Brute':
            for ip in targets:
                self.view.createNewBruteTab(ip[0], ip[1], ip[
                    3])  # ip[0] is the IP, ip[1] is the port number and ip[3] is the service name
            return
    ...
    def handleProcessAction(self, selectedProcesses,
                            action):  # selectedProcesses is a list of tuples (pid, status, procId)
    ...
    def runCommand(self, name, tabtitle, hostip, port, protocol, command, starttime, outputfile, textbox,
                   discovery=True, stage=0, stop=False):
        ...
        textbox.setProperty('dbId', QVariant(str(self.logic.addProcessToDB(
            qProcess))))  # database id for the process is stored so that we can retrieve the widget later (in the tools tab)
        ...
        if stage > 0 and stage < 5:  # if this is a staged nmap, launch the next stage
            qProcess.finished.connect(lambda: self.runStagedNmap(str(hostip), discovery, stage + 1,
                                                                 self.logic.isKilledProcess(str(qProcess.id))))
    ...
    def runStagedNmap(self, iprange, discovery=True, stage=1, stop=False):
        if not stop:
            ...
            self.runCommand('nmap', 'nmap (stage ' + str(stage) + ')', str(iprange), '', '', command,
    ...
    def nmapImportFinished(self):
        ...
        self.view.importProgressWidget.hide()  # hide the progress widget
        self.view.displayAddHostsOverlay(
            False)  # if nmap import was the first action, we need to hide the overlay (note: we shouldn't need to do this everytime. this can be improved)
    ...
    def screenshotFinished(self, ip, port, filename):
        ...
        imageviewer = self.view.createNewTabForHost(ip, 'screenshot (' + port + '/tcp)', True, '',
                                                    str(self.logic.outputfolder) + '/screenshots/' + str(filename))
        ...
        try:
            if not self.logic.isKilledProcess(str(qProcess.id)):  # if process was not killed
                if not qProcess.outputfile == '':
                    self.logic.moveToolOutput(
                        qProcess.outputfile)  # move tool output from runningfolder to output folder if there was an output file
    ...
    def handleHydraFindings(self, bWidget, userlist,
                            passlist):  # when hydra finds valid credentials we need to save them and change the brute tab title to red
                ...
                if tool[0] == "screenshooter":
                    ...
                    for a in self.settings.portActions:
                        if tool[0] == a[1]:
                            ...
                            outputfile = self.logic.runningfolder + "/" + re.sub("[^0-9a-zA-Z]", "", str(
                                tool[0])) + "/" + getTimestamp() + '-' + a[1] + "-" + ip + "-" + port
                            command = str(a[2])
