...
class BruteWidget(QtGui.QWidget):
    def __initold__(self, ip, port, service, hydraServices, hydraNoUsernameServices, hydraNoPasswordServices,
                    bruteSettings, generalSettings, parent=None):
        QtGui.QWidget.__init__(self, parent)
        self.ip = ip
        self.port = port
    ...
    def resetDisplay(
            self):  # used to be able to display the tool output in both the Brute tab and the tool display panel
...
class HostInformationWidget(QtGui.QWidget):
    ...
    def updateFields(self, status='', openPorts='', closedPorts='', filteredPorts='', ipv4='', ipv6='', macaddr='',
                     osMatch='', osAccuracy=''):
