...
class Validate(
    QtCore.QObject):  # used to validate user input on focusOut - more specifically only called to validate tool name in host/port/terminal commands tabs
...
class AddSettingsDialog(QtGui.QDialog):  # dialog shown when the user selects settings menu
    def __init__(self, parent=None):
        ...
        self.defaultServicesList = ["mysql-default", "mssql-default", "ftp-default", "postgres-default",
                                    "oracle-default"]
    ...
    def setupConnections(self):
        ...
        self.addServicesButton.clicked.connect(
            lambda: self.moveService(self.servicesAllTableWidget, self.servicesActiveTableWidget))
        self.removeServicesButton.clicked.connect(
            lambda: self.moveService(self.servicesActiveTableWidget, self.servicesAllTableWidget))
        self.addTerminalServiceButton.clicked.connect(
            lambda: self.moveService(self.terminalServicesAllTable, self.terminalServicesActiveTable))
        self.removeTerminalServiceButton.clicked.connect(
            lambda: self.moveService(self.terminalServicesActiveTable, self.terminalServicesAllTable))
        ...
        self.hostActionNameText.textChanged.connect(
            lambda: self.realTimeToolNameUpdate(self.toolForHostsTableWidget, self.hostActionNameText.text()))
        self.portActionNameText.textChanged.connect(
            lambda: self.realTimeToolNameUpdate(self.toolForServiceTableWidget, self.portActionNameText.text()))
        self.terminalActionNameText.textChanged.connect(
            lambda: self.realTimeToolNameUpdate(self.toolForTerminalTableWidget, self.terminalActionNameText.text()))
    ...
    def setSettings(self,
                    settings):  # called by the controller once the config file has been read at start time and also when the cancel button is pressed to forget any changes.
    ...
    def updateSettings(
            self):  # updates the local settings object (must be called when applying settings and only after validation succeeded)
    ...
    def populateSettings(
            self):  # called by setSettings at start up or when showing the settings dialog after a cancel action. it populates the GUI with the controller's settings object.
    ...
    def populateAutomatedAttacksTab(self):  # TODO: this one is still to big and ugly. needs work.
        ...
        for i in range(len(self.settings.portActions)):
            ...
            self.typeDic.update({self.settings.portActions[i][1]: [QtGui.QLabel(), QtGui.QLineEdit(), QtGui.QCheckBox(),
                                                                   QtGui.QHBoxLayout()]})
    ...
    def switchTabClick(
            self):  # LEO: this function had duplicate code with validateCurrentTab(). so now we call that one.
        ...
        if self.validateCurrentTab(
                self.previousTab):  # LEO: we don't care about the return value in this case. it's just for debug.
    ...
    def toggleRedBorder(self, widget,
                        red=True):  # called by validation functions to display (or not) a red border around a text input widget when input is (in)valid. easier to change stylesheets in one place only.
        if red:
    ...
    def validateBruteTab(
            self):  # LEO: do NOT change the order of the AND statements otherwise validation may not take place if first condition is False
    ...
    def validateCommandTabs(self, nameInput, labelInput,
                            commandInput):  # only validates the tool name, label and command fields for host/port/terminal tabs
        validationPassed = True

        if self.validationPassed == False:  # the self.validationPassed comes from the focus out event
            self.toggleRedBorder(nameInput,
                                 True)  # TODO: this seems like a dodgy way to do it - functions should not depend on hope :) . maybe it's better to simply validate again. code will be clearer too.
            validationPassed = False
        else:
    ...
    def validateToolName(
            self):  # called when there is a focus out event. only validates the tool name (key) for host/port/terminal tabs
        ...
        if tmplineEdit:
            ...
            if row != -1:  # LEO: when the first condition is True the validateUniqueToolName is never called (bad if we want to show a nice error message for the unique key)
                if not validateString(str(tmplineEdit.text())) or not self.validateUniqueToolName(tmpWidget, row, str(
                        tmplineEdit.text())):
    ...
    def validateUniqueToolName(self, widget, tablerow,
                               text):  # LEO: the function that calls this one already knows the selectGroup stuff so no need to duplicate.
        if tablerow != -1:
    ...
    def validateStagedNmapTab(
            self):  # LEO: renamed and fixed bugs. TODO: this function is being called way too often. something seems wrong in the overall logic
    ...
    def addToolForHost(self):
        ...
        if self.validateCommandTabs(self.hostActionNameText, self.hostLabelText, self.hostCommandText):
            ...
            self.toolForHostsTableWidget.item(self.toolForHostsTableWidget.rowCount() - 1, 0).setText(
    ...
    def addToolForService(self):
        ...
        if self.validateCommandTabs(self.portActionNameText, self.portLabelText, self.portCommandText):
            ...
            self.toolForServiceTableWidget.item(self.toolForServiceTableWidget.rowCount() - 1, 0).setText(
    ...
    def addToolForTerminal(self):
        ...
        if self.validateCommandTabs(self.terminalActionNameText, self.terminalLabelText, self.terminalCommandText):
            ...
            self.toolForTerminalTableWidget.item(self.toolForTerminalTableWidget.rowCount() - 1, 0).setText(
    ...
    def moveService(self, src,
                    dst):  # in the multiple choice widget (port/terminal commands tabs) it transfers services bidirectionally
