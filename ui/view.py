...
class View(QtCore.QObject):
    ...
    def start(self, title='*untitled'):
        ...
        self.displayAddHostsOverlay(
            True)  # displays an overlay over the hosttableview saying 'click here to add host(s) to scope'
    ...
    def initTables(self):  # this function prepares the default settings for each table
        ...
        headers = ["Id", "OS", "Accuracy", "Host", "IPv4", "IPv6", "Mac", "Status", "Hostname", "Vendor", "Uptime",
                   "Lastboot", "Distance", "CheckedHost", "State", "Count"]
        ...
        # process table
        headers = ["Progress", "Display", "Pid", "Name", "Tool", "Host", "Port", "Protocol", "Command", "Start time",
                   "OutputFile", "Output", "Status"]
    ...
    def dealWithRunningProcesses(self, exiting=False):
        if len(self.controller.getRunningProcesses()) > 0:
            ...
            reply = QtGui.QMessageBox.question(self.ui.centralwidget, 'Confirm', message,
    ...
    def dealWithCurrentProject(self,
                               exiting=False):  # returns True if we can proceed with: creating/opening a project or exiting
    ...
    def confirmExit(self):
        reply = QtGui.QMessageBox.question(self.ui.centralwidget, 'Confirm', "Are you sure to exit the program?",
                                           QtGui.QMessageBox.Yes | QtGui.QMessageBox.No, QtGui.QMessageBox.No)
    ...
    def openExistingProject(self):
        if self.dealWithCurrentProject():
            filename = QtGui.QFileDialog.getOpenFileName(self.ui.centralwidget, 'Open project',
                                                         self.controller.getCWD(), filter='SPARTA project (*.sprt)')
            ...
            if not filename == '':  # check for permissions
                if not os.access(filename, os.R_OK) or not os.access(filename, os.W_OK):
                    ...
                    reply = QtGui.QMessageBox.warning(self.ui.centralwidget, 'Warning',
                ...
                self.displayAddHostsOverlay(
                    False)  # do not show the overlay because the hosttableview is already populated
    ...
    def saveProjectAs(self):
        ...
        filename = QtGui.QFileDialog.getSaveFileName(self.ui.centralwidget, 'Save project as', self.controller.getCWD(),
                                                     filter='SPARTA project (*.sprt)',
                                                     options=QtGui.QFileDialog.DontConfirmOverwrite)

        while not filename == '':

            if not os.access(ntpath.dirname(str(filename)), os.R_OK) or not os.access(ntpath.dirname(str(filename)),
                                                                                      os.W_OK):
                print '[-] Insufficient permissions on this folder.'
                reply = QtGui.QMessageBox.warning(self.ui.centralwidget, 'Warning',
                                                  "You don't have the necessary permissions on this folder.", "Ok")
                ...
                reply = msgBox.question(self.ui.centralwidget, 'Confirm', "A file named \"" + ntpath.basename(
                    str(filename)) + "\" already exists.  Do you want to replace it?", "Abort", "Replace", "", 0)

                if reply == 1:
                    self.controller.saveProjectAs(filename, 1)  # replace
                    break

            filename = QtGui.QFileDialog.getSaveFileName(self.ui.centralwidget, 'Save project as', '.',
                                                         filter='SPARTA project (*.sprt)',
                                                         options=QtGui.QFileDialog.DontConfirmOverwrite)
    ...
    def callAddHosts(self):
        if validateNmapInput(self.adddialog.textinput.text()):
            ...
            self.controller.addHosts(self.adddialog.textinput.text(), self.adddialog.discovery.isChecked(),
                                     self.adddialog.nmap.isChecked())
    ...
    def importNmap(self):
        ...
        filename = QtGui.QFileDialog.getOpenFileName(self.ui.centralwidget, 'Choose nmap file',
                                                     self.controller.getCWD(), filter='XML file (*.xml)')
    ...
    def hostTableClick(self):
        if self.ui.HostsTableView.selectionModel().selectedRows():  # get the IP address of the selected host (if any)
            ...
            self.ui.ServicesTabWidget.setCurrentIndex(
                save)  # display services tab if we are coming from a dynamic tab (non-fixed)
    ...
    def serviceNamesTableClick(self):
        if self.ui.ServiceNamesTableView.selectionModel().selectedRows():
            row = self.ui.ServiceNamesTableView.selectionModel().selectedRows()[
                len(self.ui.ServiceNamesTableView.selectionModel().selectedRows()) - 1].row()
    ...
    def toolsTableClick(self):
        if self.ui.ToolsTableView.selectionModel().selectedRows():
            row = self.ui.ToolsTableView.selectionModel().selectedRows()[
                len(self.ui.ToolsTableView.selectionModel().selectedRows()) - 1].row()
            ...
            self.displayScreenshots(
                self.tool_clicked == 'screenshooter')  # if we clicked on the screenshooter we need to display the screenshot widget
    ...
    def scriptTableClick(self):
        if self.ui.ScriptsTableView.selectionModel().selectedRows():
            row = self.ui.ScriptsTableView.selectionModel().selectedRows()[
                len(self.ui.ScriptsTableView.selectionModel().selectedRows()) - 1].row()
    ...
    def toolHostsClick(self):
        if self.ui.ToolHostsTableView.selectionModel().selectedRows():
            row = self.ui.ToolHostsTableView.selectionModel().selectedRows()[
                len(self.ui.ToolHostsTableView.selectionModel().selectedRows()) - 1].row()
            ...
            if self.tool_clicked == 'screenshooter':
                ...
                if self.ui.DisplayWidget.findChild(
                        QtGui.QPlainTextEdit):  # remove the tool output currently in the tools display panel (if any)
                ...
                for tab in tabs:  # place the tool output textview in the tools display panel
                    if tab.findChild(QtGui.QPlainTextEdit) and str(
                            tab.findChild(QtGui.QPlainTextEdit).property('dbId').toString()) == str(
    ...
    def advancedFilterClick(self, current):
        self.filterdialog.setCurrentFilters(
            self.filters.getFilters())  # to make sure we don't show filters than have been clicked but cancelled
    ...
    def tableDoubleClick(self):
        ...
        if tab == 'Services':
            row = self.ui.ServicesTableView.selectionModel().selectedRows()[
                len(self.ui.ServicesTableView.selectionModel().selectedRows()) - 1].row()
    ...
    def switchTabClick(self):
        if self.ServiceNamesTableModel:  # fixes bug when switching tabs at start-up
            ...
            self.displayToolPanel(
                selectedTab == 'Tools')  # display tool panel if we are in tools tab, hide it otherwise
    ...
    def switchMainTabClick(self):
        ...
        self.ui.MainTabWidget.tabBar().setTabTextColor(1,
                                                       QtGui.QColor())  # in case the Brute tab was red because hydra found stuff, change it back to black

    ###
    def setVisible(
            self):  # indicates that a context menu is showing so that the ui doesn't get updated disrupting the user
        self.menuVisible = True

    def setInvisible(
            self):  # indicates that a context menu has now closed and any pending ui updates can take place now
    ...
    def contextMenuHostsTableView(self, pos):
        if len(self.ui.HostsTableView.selectionModel().selectedRows()) > 0:
            row = self.ui.HostsTableView.selectionModel().selectedRows()[
                len(self.ui.HostsTableView.selectionModel().selectedRows()) - 1].row()
            self.ip_clicked = self.HostsTableModel.getHostIPForRow(
                row)  # because when we right click on a different host, we need to select it
    ...
    def contextMenuServiceNamesTableView(self, pos):
        if len(self.ui.ServiceNamesTableView.selectionModel().selectedRows()) > 0:
            ...
            if action:
                ...
                if tool:
                    hosts = self.controller.getHostsForTool(tool,
                                                            'FetchAll')  # fetch the hosts that we already ran the tool on
                    oldTargets = []
    ...
    def contextToolHostsTableContextMenu(self, pos):
        if len(self.ui.ToolHostsTableView.selectionModel().selectedRows()) > 0:
            ...
            if port:
                ...
                menu, actions = self.controller.getContextMenuForHost(
                    str(self.HostsTableModel.getHostCheckStatusForRow(self.HostsTableModel.getRowForIp(ip))), False)
                menu.aboutToShow.connect(self.setVisible)
                menu.aboutToHide.connect(self.setInvisible)
    ...
    def contextMenuServicesTableView(self,
                                     pos):  # this function is longer because there are two cases we are in the services table
        if len(self.ui.ServicesTableView.selectionModel().selectedRows()) > 0:

            if len(
                    self.ui.ServicesTableView.selectionModel().selectedRows()) == 1:  # if there is only one row selected, get service name
                row = self.ui.ServicesTableView.selectionModel().selectedRows()[
                    len(self.ui.ServicesTableView.selectionModel().selectedRows()) - 1].row()
    ...
    def updateServiceNamesTableView(self):
        headers = ["Name"]
        self.ServiceNamesTableModel = ServiceNamesTableModel(self.controller.getServiceNamesFromDB(self.filters),
                                                             headers)
    ...
    def updateToolsTableView(self):
        if self.ui.MainTabWidget.tabText(
                self.ui.MainTabWidget.currentIndex()) == 'Scan' and self.ui.HostsTabWidget.tabText(
                self.ui.HostsTabWidget.currentIndex()) == 'Tools':
            headers = ["Progress", "Display", "Pid", "Tool", "Tool", "Host", "Port", "Protocol", "Command",
                       "Start time", "End time", "OutputFile", "Output", "Status", "Closed"]
    ...
    def updateInformationView(self, hostIP):
        ...
        if hostIP:
            ...
            if host:
                ...
                self.hostInfoWidget.updateFields(host.status, counterOpen, counterClosed, counterFiltered, host.ipv4,
                                                 host.ipv6, host.macaddr, host.os_match, host.os_accuracy)
    ...
    def updateToolHostsTableView(self, toolname):
        headers = ["Progress", "Display", "Pid", "Name", "Action", "Target", "Port", "Protocol", "Command",
                   "Start time", "OutputFile", "Output", "Status", "Closed"]
    ...
    def updateProcessesTableView(self):
        headers = ["Progress", "Display", "Pid", "Name", "Tool", "Host", "Port", "Protocol", "Command", "Start time",
                   "End time", "OutputFile", "Output", "Status", "Closed"]
        self.ProcessesTableModel = ProcessesTableModel(self, self.controller.getProcessesFromDB(self.filters, True),
                                                       headers)
    ...
    def createNewTabForHost(self, ip, tabtitle, restoring=False, content='', filename=''):
        ...
        if 'screenshot' in str(tabtitle):  # TODO: use regex otherwise tools with 'screenshot' in the name are screwed.
            ...
            if self.controller.getSettings().general_tool_output_black_background == 'True':
                ...
                tempTextView.setStyleSheet(
                    "QMenu { color:black;}")  # font-size:18px; width: 150px; color:red; left: 20px;}"); # set the menu font color: black
    ...
    def closeHostToolTab(self, index):
        ...
        if currentTabIndex >= self.ui.ServicesTabWidget.currentIndex():  # select the initially selected tab
            self.ui.ServicesTabWidget.setCurrentIndex(
                currentTabIndex - 1)  # all the tab indexes shift if we remove a tab index smaller than the current tab index
        else:
    ...
    def restoreToolTabs(self):
        tools = self.controller.getProcessesFromDB(self.filters,
                                                   False)  # false means we are fetching processes with display flag=False, which is the case for every process once a project is closed.
        ...
        for t in tools:
            if not t.tabtitle == '':
                if 'screenshot' in str(t.tabtitle):
                    ...
                    self.createNewTabForHost(t.hostip, t.tabtitle, True, t.output).setProperty('dbId', QVariant(str(
                        t.id)))  # True means we are restoring tabs. Set the widget's object name to the DB id of the process
    ...
    def restoreToolTabWidget(self, clear=False):
        ...
        if clear:
            if self.ui.DisplayWidget.findChild(
                    QtGui.QPlainTextEdit):  # remove the tool output currently in the tools display panel
                self.ui.DisplayWidget.findChild(QtGui.QPlainTextEdit).setParent(None)
    ...
    def createNewBruteTab(self, ip, port, service):
        ...
        self.ui.BruteTabWidget.setCurrentIndex(
            self.ui.BruteTabWidget.count() - 1)  # show the last added tab in the brute widget
    ...
    def callHydra(self, bWidget):
        if validateNmapInput(bWidget.ipTextinput.text()) and validateNmapInput(
                bWidget.portTextinput.text()) and validateCredentials(
                bWidget.usersTextinput.text()) and validateCredentials(bWidget.passwordsTextinput.text()):
    ...
    def bruteProcessFinished(self, bWidget):
        ...
        self.createNewTabForHost(str(bWidget.ip), str(bWidget.objectName()), restoring=True,
                                 content=unicode(bWidget.display.toPlainText())).setProperty('dbId', QVariant(
            str(bWidget.display.property('dbId').toString())))
