...
import os, sys, urllib2, socket, time, datetime, locale, webbrowser, \
...
class Screenshooter(QtCore.QThread):
    ...
    def save(self, url, ip, port, outputfile):
        ...
        command = "cutycapt --max-wait=" + str(self.timeout) + " --url=" + str(url) + "/ --out=\"" + str(
            self.outputfolder) + "/" + str(outputfile) + "\""
        #		print command
        p = subprocess.Popen(command, shell=True)
...
def sanitise(
        string):  # this function makes a string safe for use in sql query. the main point is to prevent sparta from breaking, not so much SQLi as such.
    s = string.replace('\'', '\'\'')
    return s
